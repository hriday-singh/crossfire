...
- [x] Read `docs/00-CONTRACTS.md` fully, especially §3 (SSE schema) and §4 (call sequence) — that's the exact contract you're building the routes against.
...
- [x] Stretch, only if hour 38 shows real slack and this is next in line after third-provider and document ingestion: `core/evaluators/overthinker.py` (Edge-Case Test) — same pattern as `devils_advocate.py`, no tool chain.
- [x] Otherwise: help stress-test routes/SSE against the varied inputs Dev A is running through the eval set.
